<template>
  ...
  <section class="content">
    ...
    <div class="row">
      <form @submit.prevent="createNews" class="form-horizontal w-100">
          ...
          <va-box
            ...
            <div slot="content">
              ...
              <span class="ml-3 text-danger" v-if="$v.formData.title.required && !$v.formData.title.minLength"
                    v-html=" 'Tiêu đề phải tối thiểu có 3 kí tự<br>' "/>
              <span class="ml-3 text-danger" v-if="$v.formData.title.required && !$v.formData.title.maxLength"
                    v-html=" 'Tiêu đề có tối đa có 225 kí tự<br>' "/>
...
export default {
  ...
  methods: {
    ...
    async createNews () {
      ...
      setTimeout(async () => {
        try {
          const {data, status} = await NewsService.saveNews(this.formData)
